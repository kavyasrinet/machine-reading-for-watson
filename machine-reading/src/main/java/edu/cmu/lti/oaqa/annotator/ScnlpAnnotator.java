package edu.cmu.lti.oaqa.annotator;
...
/**
 * Modified version of cleartk stanford corenlp wrapper.
 * The annotation result is cached in mongodb server
 * TODO separate wrapper and annotator
 *
 * @author Di Wang
 */
public class ScnlpAnnotator extends JCasAnnotator_ImplBase {

  public static AnalysisEngineDescription getDescription() throws ResourceInitializationException {
    return AnalysisEngineFactory.createEngineDescription(ScnlpAnnotator.class);
  }

  private StanfordCoreNLP processor = null;

  private MongoPojoCache<Annotation> cacheServer;


  static String DEFAULT_PIPELINE = "tokenize, ssplit, pos, lemma, ner, parse, dcoref";
  String pipeline = DEFAULT_PIPELINE;

  @Override
  public void initialize(UimaContext context) throws ResourceInitializationException {
    super.initialize(context);
    cacheServer = new MongoPojoCache<Annotation>(this.getClass());
  }
  ...
  public void process(JCas jCas) throws AnalysisEngineProcessException {
    ...
    try {
      ...
      addAnnotationToCas(document, jCas);
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public MongoPojoCache<Annotation> getCacheServer() {
    return cacheServer;
  }

  public void setCacheServer(MongoPojoCache<Annotation> cacheServer) {
    this.cacheServer = cacheServer;
  }

  public Annotation getAnnotation(String text) throws NoSuchAlgorithmException, IOException,
          ClassNotFoundException {
    Annotation document = (Annotation) cacheServer.get(text);
    if (document == null) {
      document = annotate(text);
      cacheServer.put(text, document);
    }
    return document;
  }

  public void initProcessor() {
    initProcessor(pipeline);
  }
  ...
  public void initProcessor(String pipeline) {
    Properties properties = new Properties();
    properties.put("annotators", pipeline);
  ...
  public void addAnnotationToCas(Annotation document, JCas jCas)
    ...
    for (CoreMap tokenAnn : document.get(TokensAnnotation.class)) {
      ...
      // hackery to convert token-level named entity tag into phrase-level
      // tag
    ...
    for (CorefChain chain : corefChains.values()) {
      ...
      for (CorefMention corefMention : chain.getMentionsInTextualOrder()) {
        ...
        // use an existing named entity mention when possible; otherwise
        // create a new one
    ...
    // add singleton entities for any named entities not picked up by
    // coreference system
    for (NamedEntityMention mention : JCasUtil.select(jCas, NamedEntityMention.class)) {
  ...
  private FSArray addTreebankNodeChildrenToIndexes(TreebankNode parent, JCas jCas,
          List<CoreLabel> tokenAnns, Tree tree) {
    Tree[] childTrees = tree.children();

    // collect all children (except leaves, which are just the words - POS
    // tags are pre-terminals in
    // a Stanford tree)
  ...
  private void addTreebankNodeToIndexes(TreebankNode node, JCas jCas, Tree tree,
    ...
    // set span, node type, children (mutual recursion), and add it to the
    // JCas
